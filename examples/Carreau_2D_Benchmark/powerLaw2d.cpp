...
T Re = 100.;      // Reynolds number
T tau = 0.64265;
T tau_carr = 0.64265;
T lx = 8.;             // channel lenght
...
T n = 0.708;              // parameter in power law model (n=1 Newtonian fluid)/ Or in the Carreau_Model
T n_carr=0.3568;
...
void prepareGeometry( CarreauUnitConverter<T,DESCRIPTOR> const& converter,
                        ...
                        int iT, SuperGeometry2D<T>& superGeometry )
{
  ...
  if ( iT==0 ) {
    ...
    AnalyticalConst2D<T,T> rho( converter.getLatticeDensity( 1060 ));
...
void error( SuperGeometry2D<T>& superGeometry,
            ...
            PowerLawUnitConverter<T,DESCRIPTOR> const& converterPL,
            Dynamics<T, DESCRIPTOR>& bulkDynamics )
{
  ...
  T distance2Wall = converterPL.getConversionFactorLength()/2.;

  PowerLaw2D<T> uSol( superGeometry,3,converterPL.getCharPhysVelocity(),distance2Wall,( n + 1. )/n );
  SuperLatticePhysVelocity2D<T,DESCRIPTOR> u( sLattice,converterPL );

  T p0 = converterPL.getPhysConsistencyCoeff()*pow( converterPL.getCharPhysVelocity(),n )*pow( ( n + 1. )/n,n )*pow( 2./( ly-distance2Wall*2 ),n + 1. );

  AnalyticalLinear2D<T,T> rho( converterPL.getLatticeDensityFromPhysPressure( -p0 ) - 1., 0, converterPL.getLatticeDensityFromPhysPressure(  p0*(lx + distance2Wall*2.)/2. ) );

  //converter.getPhysConsistencyCoeff()*pow( converter.getCharPhysVelocity(),n )*pow( ( n + 1. )/n,n )*pow( 2./( ly-distance2Wall*2 ),n + 1. );
  AnalyticalLinear2D<T,T> pressureSol( -p0, 0, p0*(lx + distance2Wall*2.)/2. );
  SuperLatticePhysPressure2D<T,DESCRIPTOR> pressure( sLattice,converterPL );
...
void getResults( SuperLattice2D<T, DESCRIPTOR>& sLattice,
                 Dynamics<T, DESCRIPTOR>& bulkDynamics,
                 CarreauUnitConverter<T,DESCRIPTOR> const& converter, int iT,
...
//   config["time"]["Tprint"].read(Tprint);
  ...
  */
  ...
  CarreauUnitConverter<T, DESCRIPTOR> const converter(
    ...
    (T)   1060.0           // physDensity: physical density in __kg / m^3__
  );

  //   PowerLawUnitConverterFrom_Resolution_RelaxationTime_Reynolds_PLindex<T, DESCRIPTOR> const converterPL(
  //   int {N},     // resolution: number of voxels per charPhysL
  ...
  clout << converter.getConversionFactorLength() << "*************" << converter.getConversionFactorTime() << std::endl;
  ...
  T p0 =1000;
  CarreauBGKdynamics<T, DESCRIPTOR> bulkDynamics( converter.getLatticeRelaxationFrequency(), instances::getBulkMomenta<T, DESCRIPTOR>(), mu_infinity, mu_zero, n, converter.getConversionFactorLength(), converter.getConversionFactorTime());

  PeriodicPressureDynamics<T, DESCRIPTOR, CarreauBGKdynamics<T,DESCRIPTOR>> outDynamics( bulkDynamics,converter.getLatticeDensityFromPhysPressure( p0*(lx + distance2Wall*2.))-1,1,0);
  PeriodicPressureDynamics<T, DESCRIPTOR, CarreauBGKdynamics<T,DESCRIPTOR>> inDynamics( bulkDynamics,-converter.getLatticeDensityFromPhysPressure( p0*(lx + distance2Wall*2. ))+1,-1,0);
